...
describe('Phase 4 & 5: Mountain Risk Intelligence Console - Verification Suite', () => {
  ...
  it('5. Renders System Status Header with live age and telemetry freshness', () => {
    ...
    expect(screen.getByText(/API: ONLINE/i)).toBeDefined();
    expect(screen.getByText(/MODEL: LOADED/i)).toBeDefined();
    expect(screen.getByText(/TELEMETRY: GOOD/i)).toBeDefined();
    expect(screen.getByText(/Age: 38m/i)).toBeDefined();
  });
  ...
  it('16. Renders Header Region Selector allowing switching between Colorado and Indian Himalayas', () => {
    let currentDomain: GeographicDomain = 'COLORADO';
    const setDomain = (d: GeographicDomain) => { currentDomain = d; };
    ...
    const selector = screen.getByLabelText(/Geographic Region Selector/i);
    expect(selector).toBeDefined();
    expect(screen.getByText(/Colorado \(Alpine Model Enabled\)/i)).toBeDefined();
    expect(screen.getByText(/Indian Himalayas \(Geographic Catalog\)/i)).toBeDefined();
    expect(currentDomain).toBe('COLORADO');
  });
});
